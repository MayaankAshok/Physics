...
class Vector:
    ...
    def getMagnSquared(self):
        return self.x ** 2 + self.y ** 2 + self.z ** 2


def toVector(mag, direction):
    import numpy as np
    x = mag * np.cos(direction / (180 / np.pi))
    y = mag * np.sin(direction / (180 / np.pi))
    return Vector(x, y)
...
def Vupdate(vec: Vector):
    vec.mag = np.sqrt(vec.x ** 2 + vec.y ** 2 + vec.z ** 2)
    if vec.x != 0:
        if vec.y != 0:
            # if x>0 and y>0:
            vec.dir = np.arctan(vec.y / vec.x) * (180 / np.pi)
            if vec.x < 0:
                vec.dir = 180 + np.arctan(vec.y / vec.x) * (180 / np.pi)
